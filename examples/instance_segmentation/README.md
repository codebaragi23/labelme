...
```bash
mindAT data_annotated --labels labels.txt --nodata --validatelabel exact --config '{shift_auto_shape_color: -2}'
mindAT data_annotated --labels labels.txt --nodata --labelflags '{.*: [occluded, truncated], person-\d+: [male]}'
```
...
```bash
mindAT_draw_label_png data_dataset_voc/SegmentationClassPNG/2011_000003.png   # left
mindAT_draw_label_png data_dataset_voc/SegmentationObjectPNG/2011_000003.png  # right
```
...
./mindAT2coco.py data_annotated data_dataset_coco --labels labels.txt
```
